...
**Method mismatch is its own answer.** `GET` at an action's path (or `POST` at a query's) is `405` with code `method_not_allowed` and an `Allow` header naming the method that verb accepts -- distinct from `404 verb_not_found`, which means no verb by that NAME exists at all. Read `hint`, switch method, retry; do not treat a 405 as "this operator does not have it".
...
**THREE CODES SAY "IT IS NOT HERE" AND THEY WANT THREE DIFFERENT MOVES FROM YOU.** They are the one place on this wire where reading the status instead of the `code` will make you do the wrong thing and report it confidently to your human. Two of them are 404 and the third is 501, and the status tells you nothing useful about which:
- **`404 verb_not_found`** -- no verb by that NAME is registered at this operator. Its `hint` lists the names that ARE. **Re-read `hint`, or re-fetch `GET <endpoint>/schema`, and call a name that exists.** Do NOT tell your human this operator cannot do the thing: you have not established that, and another verb may well do it.
...
The reason this is spelled out at length: until 0.4.12 all three were one code, and an assistant told "not found" about a hotel that does not exist would re-read the catalogue, retry, and then tell its human the operator was broken. One of those three moves was right and it was not that one.
...
```json
{
  "kiosk": {
    "skill": {
      "url": "https://kiosk.tech/skill-v0.4.12.md",
      "sha256": "abc123..."
    }
  }
}
