...
**The transacting verbs require auth:** send the access token from Step 2 as `Authorization: Bearer <access_token>` on every `query`/`run`/`pay` call -- the operator answers `401` without it. **`schema` is the exception: it is anonymously readable** (it is just the machine-readable API description), so you MAY read it *before* you register, to learn what the operator offers -- send your token if you have one, but it is not required. (An operator MAY still answer `402 pow_required` on `schema` -- that is the PoW gate, separate from auth.) `POST /auth/revoke` also requires the Bearer header (it identifies the session to keep); `challenge`/`register`/`login` do not (you have no token yet).
...
`GET <endpoint>/schema` -> the operator's queries and actions, each with params and a free-text `description`. **Anonymously readable** -- no Bearer token required (send one if you have it); you may read it before registering, to learn what the operator offers. Read the descriptions -- they tell you what this operator actually does; do not assume names.

```python
# schema is anonymously readable -- no Authorization header needed
...
- **Proof-of-work:** any verb -- `schema`, `query`, `run`, `pay` -- as well as `POST /auth/register`, may return HTTP 402 `pow_required` (an operator MAY toll any of them; the only always-free entrypoint is the top-level discovery at `/.well-known/kiosk.json`). Solve every challenge and retry the same body with the `pow` field. `POST /pay` can instead 402 with `payment_setup_required` (no `challenges`) -- that one means run `payment_setup`, not solve PoW. Branch on `error.code` (see "Two kinds of 402").
