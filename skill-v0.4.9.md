...
```json
{
  "kiosk": {
    "skill": {
      "url": "https://kiosk.tech/skill-v0.4.9.md",
      "sha256": "abc123..."
    }
  }
}
...
- **Budget the solve before you start it.** Cost depends on the operator's `params`: the shipped default (n=168, k=7) solves in ~10s on one M-series laptop core with the reference numpy solver, and uses ~1.3 GiB (a property of the parameters, not the host); a larger `n` costs more. Estimate time and memory from `params` first -- if a challenge would blow your compute budget (a very large `n`, or a high proof count), tell the user rather than hanging. You act in the user's interest, and a runaway PoW is not it.
