...
- **a topic covers it** -> subscribe: that is how you FIND OUT, and HOW you find out depends on which KIND of topic it is; see "How to wait" below. Do NOT also poll -- a loop running beside a stream is exactly the cost the stream exists to delete.
...
Leave `subject` OFF to take everything on that topic that is yours; name it to narrow to one row. A subscription the operator refuses -- a topic name it does not publish, or a subject its own rule says you may not read -- comes back **`reject_subscription`**, and never as a subscription that is simply silent, so it is something you can act on -- and it is a mistake of yours, never a transient: fix the name from the catalogue, name the subject the operator's own row gave you, or fall back to the poll. Retrying the same subscription unchanged gets the same refusal for ever. (An operator does not publish the rule by which it decides a subject is yours; do not go looking for it.)
...
- **`truncated: true`** means the operator cannot prove you have seen everything after the `since` you presented. **Re-read the current state through the ordinary verb that owns it, exactly ONCE, then carry on with the stream.** It is not an error, not a reason to re-subscribe, and not a reason to walk `since` backwards looking for the gap -- what the operator no longer retains it cannot hand you at any cursor.
...
- `{"type":"disconnect","reason":"revoked","reconnect":false}` -- the credential itself was revoked. **`reconnect: false` is TERMINAL for this token** -- do not back off and retry, whatever your reconnect logic would otherwise do: the answer cannot change, and retrying into it is a reconnect storm against the origin. Get a fresh token the way Step 2 does and open a new stream -- and if `/auth/login` answers `404`, the human unlinked you, so stop and ask them.
...
**Fetch `https://kiosk.tech/events/listen-v0.5.1.py`, verify its SHA-256 equals `2d34ae0cc75a8143ea95f876c003a00e1ca002e6868403630f422ebb8f34c0b5`, and only then execute it.** (`https://kiosk.tech/events/listen.py` always serves the current listener and is what an operator's message may point you at; prefer the pinned URL above, and never run either copy without checking the hash.) It needs `websockets>=12` from PyPI -- put it in the same venv this skill already has you create.

Its output is one JSON object per line, carrying the frames above plus `{"type":"open"}` and `{"type":"reconnecting","in":...,"since":...}`. **Its EXIT CODE is an answer you act on**, so read it rather than only the lines: `0` an event arrived, or your `--max-seconds` ran out; `5` a `--until-event` wait hit its deadline with nothing -- wait again; `3` the operator will not take this token back, so get a fresh one (Step 2) and start over; `4` every topic you named was refused, so fix the name or the subject -- that run would have waited for ever.

```
python3 listen-v0.5.1.py --url "$EVENTS_URL" --token "$ACCESS_TOKEN" \
...
**Two kinds of topic, waited on differently. Decide which one you have before you start; neither procedure works for the other.**

- **A WAIT** -- you are sitting here, holding on for an answer to something you just asked for: an identity check, a card being saved, a desk confirming a booking. It lands in seconds or minutes, while you are still running.
- **A SUBSCRIPTION** -- the event lands hours or days later, when you are not running at all: a delivery setting off, a shared list somebody else edits.

**A WAIT: block in the FOREGROUND, inside one tool call.**

```
python3 listen-v0.5.1.py --url "$EVENTS_URL" --token "$ACCESS_TOKEN" \
        --topic kyc_verification --until-event --max-seconds 300
```

It prints the first event and exits `0` the moment it arrives, so the wait costs what the human took rather than the deadline. **Exit `5` is the deadline, not an answer: start it again.** Only when that flow's give-up horizon has passed -- ~10 minutes for an identity check -- do you stop and ask the human. **And run one more wait before you tell them anything is still pending**, because the minute you were about to skip is usually the minute they finished in.

**NEVER background the listener into a log file and poll that file between turns.** Nothing wakes you when a line lands in a file, so the event arrives and nobody reads it -- worse than the poll the stream replaces. If your runtime cannot hold one tool call open for minutes, run SHORT foreground waits back to back (`--max-seconds 60`, one per turn, until the horizon) -- still foreground, never a file.

**A SUBSCRIPTION: the CURSOR delivers it, not the socket.** Keep the highest `id` you have seen -- `head` from the subscription confirmation, then every event's `id` -- somewhere that outlives your turn. Next time you run, start the listener with `--since <that id>` and take everything that landed while you were gone. This needs no held socket and no long-lived process, and for a turn-based assistant it is the PRIMARY mechanism for this kind of topic, not a fallback.

**A background process is worth something only if it can WAKE you.** Where your runtime lets a subagent or a long-running command signal you, hold one and let it tell you when an event arrives; keep the cursor as well, because that is what covers the gap when it cannot. A background process that writes to a log nobody is woken by is worth exactly as much as no process at all.

**Do not tell your human you will be told.** Say that only while you are actually holding something that will wake you -- a foreground wait you have not returned from, or a helper your runtime can signal you from. If you are holding neither, the true sentence is that you will check when they next speak, and it is the one to say. A promise of notification you cannot keep is worse than asking them to come back.
...
```json
{
  "kiosk": {
    "skill": {
      "url": "https://kiosk.tech/skill-v0.5.0.md",
      "sha256": "abc123..."
    }
  }
}
...
**Backward compatibility -- and why it does not protect you yet.** From protocol **1.0** onward, versions within a MINOR series are backward-compatible: new endpoints and fields are additive, existing flows never break. **The protocol is 0.5. Before 1.0 that promise does not bind, so ANY release may change the wire -- a PATCH included, up to removing a response field.** 0.4.1 did: it moved a paginating query's cursor out of the body into a `Link` header, so a 0.4.0 skill looks for a `next` field no 0.4.1 operator sends. What protects you instead is the pin, and it is the step above this paragraph: **the operator names one exact `skill-vX.Y.Z.md` plus its SHA-256, and you adopt that cut before you transact.** `Kiosk-Min-Client` tells you when you are behind, but it is advisory -- no endpoint refuses you on it, so it warns and never blocks. So: never assume a patch is safe to skip because the numbers look close; do the dual-check and adopt what the operator pins. ACROSS a MINOR the wire MAY break in every series, and it has twice: 0.3 -> 0.4 gave every verb its own endpoint and removed the multiplexed `POST /query` and `POST /run` together with the response envelope, and 0.4 -> 0.5 added a third array to the catalog, `events`, which is REQUIRED and whose root is closed -- so a 0.4 reader refuses the catalog a 0.5 origin serves. If you are holding an older skill than the operator pins, you cannot transact until you adopt the pinned cut.
