...
version: "0.2.4"
description: "Universal protocol for agent-native commerce. Talk to any Kiosk-speaking operator — register, discover its capabilities via schema, then act: order, book, apply, pay."
...
### Step 6: Pay
Sign 3 RS256 JWS mandates (intent → cart → payment). `iss` must match the `issuer` from `/.well-known/kiosk.json` (under the `kiosk` key) verbatim. Submit via `POST <endpoint>/pay {intent_mandate_jws, cart_mandate_jws, payment_mandate_jws}` with the Bearer header. Payment mandate: `payment_method: "on_file"` for saved-card flow. A `402 payment_setup_required` here means no saved card — run `payment_setup` (Step 5), then retry (see "Two kinds of 402"). A `403 spending_cap_exceeded` means the human has set a per-assistant spending cap that this purchase would exceed — you CANNOT pay past it; tell the human to raise this assistant's spending limit in their provider account, then retry (see "Spending cap (HTTP 403)").
...
**Backward compatibility.** Within a MINOR series (0.1.x, 0.2.x, …) versions are backward-compatible — new endpoints and fields are additive, existing flows never break. An agent on a newer patch version can transact with a provider pinning an older one; an agent on an older patch MUST update before transacting with a provider pinning a newer one. The skill's MAJOR.MINOR tracks the protocol version (version parity).
...
`POST /pay` MAY also come back `403` with `error.code: "spending_cap_exceeded"` — the human has set a per-assistant spending cap in their provider account, and this purchase would push you past it (a cap of `0` disables the assistant entirely). This is not a setup or PoW gate: no `payment_setup`, no challenge, nothing you can solve — you CANNOT pay past the cap. Tell the human to raise this assistant's spending limit in their provider account, then retry once they have. Branch on `error.code` here too — a `403` is a hard stop from the human's own policy, distinct from the two `402` gates above.
