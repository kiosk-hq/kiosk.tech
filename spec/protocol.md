...
4. **Skill version.** The skill is published as `skill-vMAJOR.MINOR.PATCH.md`, where
   **MAJOR.MINOR tracks the protocol release** (currently 0.4, so version parity holds)
   ...
   wire itself may change in a PATCH. The current skill is **0.4.8**. Every cut
   ...
   cannot transact with a 0.4 origin at all, and 0.4.0-0.4.7 describe
   earlier 0.4 cuts that a 0.4.8 operator no longer serves. Published skill
